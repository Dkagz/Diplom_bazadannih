...
namespace InventoryManagementSystem
{
    public partial class ProductForm : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            TextBox txtSearch1 = txtSearch;
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE CONCAT(pid, pname, pprice, pdescription, pcategory) LIKE '%" + txtSearch1.Text+"%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        {
            ProductPrint productForm = new ProductPrint();
